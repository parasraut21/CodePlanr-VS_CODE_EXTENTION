...
import { ChatMessage, SimpleChatProvider } from './simpleChatProvider';
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(
      message => {
        switch (message.type) {
          case 'sendMessage':
            this.chatProvider.sendMessage(message.text).then(() => {
              this.updateWebview();
            });
            break;
          case 'clearChat':
            ...
            break;
          case 'generateCode':
            this.chatProvider.generateCodeForPlan(message.plan).then(() => {
              this.updateWebview();
            });
            break;
          case 'applyChanges':
            this.chatProvider.applyPlanChanges(message.plan).then(() => {
              this.updateWebview();
            });
            break;
          case 'applyStep':
            this.chatProvider.applyStepChanges(message.step, message.changes).then(() => {
              this.updateWebview();
            });
            break;
        }
    ...
    // Update webview when messages change
    // We'll update manually when needed
  }
